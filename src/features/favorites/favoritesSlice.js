...
const persisted = JSON.parse(localStorage.getItem("tt_favorites") || "[]");
const initialState = { list: persisted };
...
const favoritesSlice = createSlice({
  ...
  reducers: {
    addFavorite(state, action) {
      ...
      localStorage.setItem("tt_favorites", JSON.stringify(state.list));
    },
    removeFavorite(state, action) {
      state.list = state.list.filter((id) => id !== action.payload);
      localStorage.setItem("tt_favorites", JSON.stringify(state.list));
